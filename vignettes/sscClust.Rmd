...
```{r }
sce.all <- ssc.run(sce, subsampling=F, k.batch=5)
```
or with this command for clustering with subsampling:
```{r message=F,warning=F}
sce.sub <- ssc.run(sce, subsampling=T, sub.frac = 0.8, k.batch=5)
```
...
```{r fig.width=8,fig.height=10,fig.wide=T}
...
sce.Pollen.ori <- ssc.reduceDim(sce.Pollen.ori,method="tsne",tSNE.usePCA = F)
ssc.plot.tsne(sce.Pollen.ori,columns = "true_labs",reduced.name = "tsne")
```

The visualization (tSNE) using PCA transformed data:
```{r fig.width=6,fig.height=5}
sce.Pollen.proj <- ssc.reduceDim(sce.Pollen,method="pca")
...
```{r fig.width=6,fig.height=5}
sce.Pollen.proj <- ssc.reduceDim(sce.Pollen.proj,method="iCor")
...
sce.Pollen.ori <- ssc.clust(sce.Pollen.ori, method.reduction="none", method="kmeans", k.batch=11)
toc()
```
...
sce.Pollen.proj <- ssc.clust(sce.Pollen.proj, method.reduction="pca", method="kmeans", k.batch=11)
toc()
```
...
sce.Pollen.proj <- ssc.clust(sce.Pollen.proj, method.reduction="iCor", method="kmeans", k.batch=11)
toc()
```
